...
class Game {
  constructor() {
    ...
    this.multiplierContainer = document.getElementById("multiplier");
    this.startButton = document.getElementById("start-button");
    this.instructions = document.getElementById("instructions");
    this.score = 0;
    this.multiplier = 1;
    this.scoreContainer.innerHTML = this.score;
    this.multiplierContainer.innerHTML = `×${this.multiplier}`;
  ...
  updateScore() {
    let blockCount = this.blocks.length - 1; // Текущий счет - количество размещенных блоков
    let newMultiplier = Math.floor((blockCount - 1) / 10) + 1; // Определение множителя

    // Обновляем множитель и отображаем его
    if (newMultiplier > this.multiplier) {
        this.multiplier = newMultiplier;
        this.multiplierContainer.innerHTML = `×${this.multiplier}`;
        this.multiplierContainer.style.opacity = 1; // Показываем множитель
    }
